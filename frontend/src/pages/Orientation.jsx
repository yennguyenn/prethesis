...
export default function Orientation() {
      ...
      <div className='min-h-screen py-12 px-4' style={{ background: 'var(--surface-muted)' }}>
        <div className='max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8'>
          <div className='text-center mb-8'>
            ...
            <h2 className='text-3xl font-bold mb-2' style={{ color: "var(--brand-blue)" }}>Hoàn thành định hướng</h2>
            <p className='text-gray-600'>Ngành phù hợp dựa trên sở thích của bạn:</p>
          </div>
          <div className='bg-primary-100 rounded-xl p-6 mb-6'>
            <h3 className='text-2xl font-bold mb-2' style={{ color: "var(--brand-blue)" }}>{result.recommended?.name || 'Chưa xác định rõ ngành phù hợp'}</h3>
          ...
          <div className='flex flex-col sm:flex-row gap-4'>
            ...
            {result.recommended?.code === 'ICT' && (
              <button onClick={() => navigate('/quiz')} className='flex-1 px-6 py-3 text-white font-semibold rounded-lg transition-colors' style={{ background: "var(--brand-blue)" }}>Tiếp tục chọn chuyên ngành IT</button>
            )}
            <button onClick={() => navigate('/')} className='flex-1 px-6 py-3 text-white font-semibold rounded-lg transition-colors' style={{ background: "var(--brand-blue)" }}>Trang chủ</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className='min-h-screen py-8 px-4' style={{ background: 'var(--surface-muted)' }}>
      <div className='max-w-3xl mx-auto'>
        <div className='bg-white rounded-t-2xl shadow-lg p-6'>
          <div className='flex justify-between items-center mb-4'>
            <h2 className='text-2xl font-bold' style={{ color: "var(--brand-blue)" }}>Bài đánh giá định hướng ngành</h2>
        ...
        {currentQuestion && (
          <div className='bg-white rounded-b-2xl shadow-lg p-8 mb-6'>
            <h3 className='text-xl font-semibold mb-6' style={{ color: "var(--brand-blue)" }}>{currentQuestion.text}</h3>
            <div className='space-y-3'>
              {(currentQuestion.options || []).map(opt => (
                <label key={opt.id} className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${answers[currentQuestion.id] === opt.id ? '' : 'border-gray-200 hover:border-[#c6d8ff] hover:bg-[#eef4ff]'}`} style={answers[currentQuestion.id] === opt.id ? { borderColor: 'var(--brand-purple)', background: 'rgba(100,116,139,0.08)' } : {}}>
                  <div className='flex items-center'>
                    <input type='radio' name={`q_${currentQuestion.id}`} checked={answers[currentQuestion.id] === opt.id} onChange={() => choose(opt.id)} className='w-5 h-5 focus:ring-primary-300' style={{ accentColor: 'var(--brand-purple)' }} />
        ...
        <div className='flex justify-between items-center'>
          ...
          {currentIndex < questions.length - 1 ? (
            <button onClick={next} disabled={!isAnswered} className='px-6 py-3 text-white font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md' style={{ background: "var(--brand-blue)" }}>Tiếp →</button>
          ) : (
            <button onClick={submit} disabled={submitting || Object.keys(answers).length < questions.length} className='px-8 py-3 text-white font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md flex items-center gap-2' style={{ background: "var(--brand-red)" }}>
