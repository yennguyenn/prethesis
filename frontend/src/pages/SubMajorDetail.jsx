...
export default function SubMajorDetail() {
  ...
  const renderJobs = (jobs) => {
    ...
    return (
      <section className="mt-8">
        <h2 className="text-lg font-semibold mb-3" style={{ color: "var(--brand-blue)" }}>Nghề tiêu biểu</h2>
  ...
  if (loading) return <div className="max-w-3xl mx-auto py-16 px-4"><h1 className="text-2xl font-bold" style={{ color: "var(--brand-blue)" }}>Đang tải...</h1></div>;
  if (error) return (
    <div className="max-w-3xl mx-auto py-16 px-4">
      <h1 className="text-2xl font-bold mb-2" style={{ color: "var(--brand-red)" }}>Lỗi</h1>
      <p className="text-slate-600 mb-6">{error}</p>
      <Link to="/careers" className="px-5 py-2 rounded-lg text-white font-medium transition" style={{ background: 'var(--brand-blue)' }}>Quay về danh sách ngành</Link>
    </div>
  );
  if (!data) return null;
  ...
  return (
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto bg-white/90 backdrop-blur rounded-3xl shadow-lg p-8 border border-slate-100">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold mb-2" style={{ color: "var(--brand-blue)" }}>{data.name}</h1>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs inline-block px-3 py-1 rounded-full border" style={{ background: '#eef1f5', color: '#444c5c', borderColor: '#bac3cf' }}>Mã: {data.code}</span>
            {data.Major && (
              <Link to={`/careers/${data.Major.code}`} className="text-xs inline-block px-3 py-1 rounded-full border" style={{ background: '#eff7f6', color: '#78a5a3', borderColor: '#b8dedd' }}>
        ...
        <section className="space-y-3">
          <h2 className="text-lg font-semibold" style={{ color: "var(--brand-ocean)" }}>Mô tả</h2>
        ...
        <div className="mt-10 flex gap-4">
          ...
          {data.Major && <Link to={`/careers/${data.Major.code}`} className="flex-1 text-center px-5 py-3 rounded-xl text-white text-sm font-semibold shadow hover:shadow-md transition" style={{ background: 'var(--brand-red)' }}>Xem ngành {data.Major.name}</Link>}
        </div>
      </div>
    </div>
  );
