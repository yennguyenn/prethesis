  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-primary-900">Kết quả người dùng</h3>
        <input
          ...
          <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm mb-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-sm font-semibold">Bài nộp #{s.id}</div>
                {s.user && (
                  <div className="text-xs text-slate-600">{s.user.name} · {s.user.email} · {s.user.role}</div>
                )}
              </div>
              <div className="px-2 py-1 rounded bg-primary-100 text-primary-700 text-xs border border-primary-300">Điểm: {s.score ?? '—'}</div>
            </div>

            <div className="grid md:grid-cols-2 gap-3 mt-3">
              <div className="rounded-lg border border-slate-200 p-3">
                <div className="text-[11px] uppercase tracking-wide text-slate-500">Ngành</div>
                <div className="text-sm font-medium">{s.majorName || s.majorCode || '—'}</div>
              </div>
              <div className="rounded-lg border border-slate-200 p-3">
                <div className="text-[11px] uppercase tracking-wide text-slate-500">Chuyên ngành</div>
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-4">
              ...
              <div className="flex flex-col gap-3">
                <div className="rounded-lg border border-slate-200 p-3">
                  {rawEntries.length === 0 ? (
                    <div className="text-sm text-slate-500">Không có điểm thô.</div>
                  ) : (
                    <div>
                      <h4 className="font-semibold text-primary-700 mb-2">Điểm thô</h4>
                      <table className="table-auto border mb-2 text-xs w-full">
                        <thead>
                          <tr>
                            <th className="border px-2">Ngành</th>
                            <th className="border px-2">Điểm thô</th>
                          </tr>
                        </thead>
                        <tbody>
                ...
                <div className="rounded-lg border border-slate-200 p-3 overflow-auto">
                  {decisionMatrix ? (
                    <div>
                      <h4 className="font-semibold text-primary-700 mb-2">Ma trận quyết định</h4>
                      <table className="table-auto border mb-2 text-xs w-full">
                        <thead>
                          <tr>
                            <th className="border px-2">Ngành</th>
                    ...
                    <div className="text-sm text-slate-500">Không có ma trận quyết định.</div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="rounded-lg border border-slate-200 p-3 overflow-auto">
                  {normalizedMatrix ? (
                    <div>
                      <h4 className="font-semibold text-primary-700 mb-2">Ma trận chuẩn hoá (0–1)</h4>
                      <table className="table-auto border mb-2 text-xs w-full">
                        <thead>
                          <tr>
                            <th className="border px-2">Ngành</th>
                    ...
                    <div className="text-sm text-slate-500">Không có ma trận chuẩn hoá.</div>
                  )}
                </div>

                <div className="rounded-lg border border-slate-200 p-3 overflow-auto">
                  {ranking.length === 0 ? (
                    <div className="text-sm text-slate-500">Không có xếp hạng SAW.</div>
                  ) : (
                    <div>
                      <h4 className="font-semibold text-primary-700 mb-2">Xếp hạng cuối</h4>
                      <table className="table-auto border mb-2 text-xs w-full">
                        <thead>
                          <tr>
                            <th className="border px-2">Hạng</th>
                            <th className="border px-2">Ngành</th>
